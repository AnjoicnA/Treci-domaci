...
<body>
    ...
    <script>
        ...
        async function showEditModal(id){
            ...
            let response = await fetch("api/user.php?id="+id);
            ...
            if(responseJSON.status == true){
                ...
                document.getElementById("editUserCity").value = userData.city;
                document.getElementById("editModalEmail").value = userData.email;

                
            filterCities(userData.country, 1) //pozivam funkciju ovdje da bih mogla da biram gradove bez klika na drzavu ako je ona vec izabrana
            }
            editModal.show();
        }


    async function filterCities(countryName, fileType){
        let response = await fetch("filter_cities.php?cname="+countryName);
        let responseJSON = await response.json();
        if(fileType == 0){
            var citySelect = document.getElementById("city");
        }
        else if(fileType == 1){
            var citySelect = document.getElementById("editUserCity");
        }
        

        console.log(citySelect)
        //prvo cistim sadrzaj select-a
        citySelect.innerHTML = "";

        //kreiram i dodajem option koji mi govori sta biram
        let basicOption = document.createElement("option");
        basicOption.value = null;
        basicOption.innerHTML = "Grad";
        basicOption.dissabled = true;
        basicOption.selected = true;
        basicOption.hidden = true;

        citySelect.append(basicOption);

        for(let i = 0; i < responseJSON.data.length; i++){
            //za svaki grad mi napravi novi option tag i dodaj ga u drop-down (select)
            let newOption = document.createElement("option");
            newOption.value = responseJSON.data[i];
            newOption.innerHTML = responseJSON.data[i];
